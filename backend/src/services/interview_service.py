...
class InterviewService:
    ...
    def start_interview(self, db: Session, interview_id: int) -> Dict:
        ...
        try:
            ...
            if questions:
                ...
                if interview.interview_type == 'audio':
    ...
    def get_next_question(self, db: Session, interview_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            # Gerar áudio da pergunta se necessário
            if interview.interview_type == 'audio':
                audio_path = self._generate_question_audio(question_data['question'], interview.id, current_index)
    ...
    def process_response(self, db: Session, interview_id: int, response_text: str, 
        ...
        try:
            ...
            questions[current_index].update({
                'response': response_text,
            ...
            is_last_question = interview.current_question_index >= len(questions)
            
            return {
                'success': True,
                'is_last_question': is_last_question,
                'next_question_index': interview.current_question_index,
                'progress': (interview.current_question_index / len(questions)) * 100,
                'audio_analysis': audio_analysis,
                'content_analysis': content_analysis
            }
    ...
    def finalize_interview(self, db: Session, interview_id: int) -> Dict:
        ...
        try:
            ...
            questions = interview.get_questions_list()
            overall_analysis = self._analyze_complete_interview(questions, interview.position)
            ...
            return {
                ...
                'next_steps': self._generate_next_steps(interview.recommendation, interview.overall_score)
            }
